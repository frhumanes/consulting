...
from wtdeploy.modules import fab_nginx
...
def deploy_info():
    ...
    run("svn info %s/app >> %s/app/media/deploy_info.txt" \
        % (env.deploy_folder, env.deploy_folder))


def quick_deploy():
    """
        this target run a quick deploy, only update source from repository
        and restart app.wsgi this target do NOT run migrations, upload
        configurations files or restart apache
    """
    with cd(env.deploy_folder):
        fab_django.deploy()
    fab_django.restart()
